...
unsigned long FramePool::bitMap[240];
...
FramePool::FramePool(unsigned long _base_frame_no, unsigned long _nframes, unsigned long _info_frame_no){
  ...
  // Setting the status of frames to 0 i.e., free(not filled)
  if(baseFrameNo<1024){
	for(i=0;i<(_nframes/32);i++){
		bitMap[i] = 0;
  	}
	for(j=0;j<(_nframes%32);j++){
		bitMap[i] = bitMap[i] & (0<<j);
	}
  }
  else{
...
unsigned long FramePool::get_frame(){
  unsigned int i,j;
  // Checking if a frame is available using the bitMap and returning the free frame
  if(this->baseFrameNo <1024){
  for(i=0;i<16;i++){
	j=0;
		while(j<32){
			if((bitMap[i] & (1<<j)) == 0){
...
void FramePool:: release_frame(unsigned long _frame_no){
  unsigned int i,j=0;
  i = (_frame_no-512)/32;
  bitMap[i] = bitMap[i] & (!(1<<(_frame_no%32)));	
}
