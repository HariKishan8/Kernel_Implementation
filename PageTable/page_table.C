...
/* STATIC MEMBERS ARE COMMON TO ENTIRE PAGING SUBSYSTEM */
  ...
  const unsigned int PageTable::ENTRIES_PER_PAGE ; /* in entries*/
...
PageTable::PageTable(){
  page_directory = (unsigned long *) ((kernel_mem_pool->get_frame())*PAGE_SIZE); // Setting up the page directory
  unsigned long *page_table = (unsigned long *) ((process_mem_pool->get_frame())*PAGE_SIZE); //Setting up the Page Table
  unsigned long address = 0; // holds the physical address of where a page is
  unsigned int i;
  // Mapping of the first 4MB of memory
  ...
  page_directory[0] = (unsigned long)page_table; // attribute set to: supervisor level, read/write, present(011)
  page_directory[0] = page_directory[0] | 3;
  for(i=1; i<ENTRIES_PER_PAGE; i++)
  {
	page_directory[i] = 0 | 2; // attribute set to: supervisor level, read/write, not present(010)
  }
...
void PageTable::enable_paging(){
  ...
  write_cr3((unsigned long)current_page_table->page_directory); // load the page directory into CR3 register
  write_cr0(read_cr0() | 0x80000000); // set the paging bit in CR0 to 1
}

/* The page fault handler. */
void PageTable::handle_fault(REGS * _r){
  unsigned long fault_addr = read_cr2(); // reading the fault address stored in CR2 register
  unsigned long addr;
  unsigned long pg_dir_idx = (fault_addr & 0xFFC00000) >> 22; 
  unsigned long pg_tb_idx = (fault_addr & 0x003FF000 ) >> 12;
  unsigned long *page_table;
  page_table = (unsigned long *) (current_page_table->page_directory[pg_dir_idx]);
 
  // checking the present bit in page directory
  if(((current_page_table->page_directory[pg_dir_idx]) & 1) == 0){
	page_table = (unsigned long *) ((process_mem_pool->get_frame())*PAGE_SIZE);
	current_page_table->page_directory[pg_dir_idx] = (unsigned long) (page_table);
	current_page_table->page_directory[pg_dir_idx] |= 3;
	page_table = (unsigned long *) ((pg_dir_idx<<12)|0xFFC00000);
	for(unsigned int i=0;i<ENTRIES_PER_PAGE;i++){
		page_table[i] = 0|2;
	}
  }
  page_table = (unsigned long *) ((pg_dir_idx<<12)|0xFFC00000);
  // checking the valid bit in the page table
  if((page_table[pg_tb_idx] & 1) == 0){
 	addr = ((process_mem_pool->get_frame())*PAGE_SIZE);
	page_table [pg_tb_idx] = addr | 3;
  }
}
